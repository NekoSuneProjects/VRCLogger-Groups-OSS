...
const { resolveUserByName, listKnownNames } = require("./groupCache");
...
function createSetupApi(profile) {
  try {
    return createVrchatApi(profile.config, profile.db);
  } catch (err) {
...
/**
 * Cached group members. Departed members are included by default (active=false)
 * so historical display names stay visible; pass activeOnly=true to hide them.
 */
async function loadGroupMembers(profile, group, query = {}) {
  const model = profile.db?.VRCGroupMemberCache;
  const groupId = normalizeGroupId(group?.groupid);
  if (!model || !groupId) return { rows: [], total: 0, limit: 0, offset: 0 };
  ...
  const where = { groupId };
  if (String(query.activeOnly || "") === "true") where.active = true;
  if (q) {
    where[Op.or] = [
  ...
  const [total, rows] = await Promise.all([
    ...
    safeFindAll(model, {
      where,
      order: [["active", "DESC"], ["displayName", "ASC"]],
      limit,
      offset
    })
  ]);

  const history = profile.db?.VRCUserNameHistory;
  const out = [];
  for (const row of plainRows(rows)) {
    const previousNames = history
      ? plainRows(await safeFindAll(history, {
          where: { userId: row.userId },
          order: [["observedAt", "ASC"], ["id", "ASC"]]
        })).map(entry => entry.oldDisplayName)
      : [];
    ...
    out.push({
      ...
      roleIds: row.roleIds || [],
      isRepresenting: Boolean(row.isRepresenting),
      joinedAt: row.joinedAt,
      firstSeenAt: row.firstSeenAt,
      lastSeenAt: row.lastSeenAt,
      leftAt: row.leftAt,
      membershipStatus: row.membershipStatus,
      previousNames: [...new Set(previousNames.filter(Boolean))]
    });
  }
...
function startDashboard({ loaded, client, defaultPort } = {}) {
  ...
  app.get("/api/guilds/:guildId/members", requireAuth, asyncRoute(async (req, res) => {
    const resolved = await resolveProfileFromRequest(req, res);
    if (!resolved) return;
    return res.json(await loadGroupMembers(resolved.profile, resolved.group, req.query));
  }));

  app.get("/api/guilds/:guildId/members/resolve", requireAuth, asyncRoute(async (req, res) => {
    const resolved = await resolveProfileFromRequest(req, res);
    if (!resolved) return;
    const match = await resolveUserByName(resolved.profile.db, req.query.name);
    if (!match) return res.status(404).json({ error: "No user matches that display name." });
    return res.json({ ...match, knownNames: await listKnownNames(resolved.profile.db, match.userId) });
  }));
  ...
  app.get("/api/client/members", asyncRoute(async (req, res) => {
    const resolved = await resolveClientApiRequest(req, res);
    if (!resolved) return;
    const payload = await loadGroupMembers(resolved.profile, resolved.group, req.query);
    return res.json({ status: 200, message: "Group members.", ...payload });
  }));

  // Backtrack a display name (current or historical) to a stable user id.
  app.get("/api/client/resolve-name/:name", asyncRoute(async (req, res) => {
    const resolved = await resolveClientApiRequest(req, res);
    if (!resolved) return;
    const match = await resolveUserByName(resolved.profile.db, req.params.name);
    if (!match) {
      return res.status(404).json({ status: 404, message: "No user matches that display name." });
    }
    return res.json({
      status: 200,
      message: match.historical ? "Matched a previous display name." : "Matched the current display name.",
      ...match,
      knownNames: await listKnownNames(resolved.profile.db, match.userId)
    });
  }));

  app.get("/api/client/names/:userId", asyncRoute(async (req, res) => {
    const resolved = await resolveClientApiRequest(req, res);
    if (!resolved) return;
    const userId = String(req.params.userId || "").trim();
    return res.json({
      status: 200,
      message: "Known display names.",
      userId,
      knownNames: await listKnownNames(resolved.profile.db, userId)
    });
  }));
