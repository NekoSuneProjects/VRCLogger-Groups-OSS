...
async function makeDatastores(config) {
  ...
  // Snapshot of the tracked VRChat group itself, refreshed by the group cache poller.
  const VRCGroupCache = GamingClientSequelize.define("vrc_group_cache", {
    id: { type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true, allowNull: false },
    groupId: { type: DataTypes.STRING(128), allowNull: false, unique: true },
    name: { type: DataTypes.STRING(500), allowNull: true },
    shortCode: { type: DataTypes.STRING(128), allowNull: true },
    discriminator: { type: DataTypes.STRING(32), allowNull: true },
    description: { type: DataTypes.TEXT, allowNull: true },
    ownerId: { type: DataTypes.STRING(128), allowNull: true },
  ...
  // One row per member ever seen in the group. Rows are kept after a member
  // leaves (active=false) so old display names stay resolvable.
  const VRCGroupMemberCache = GamingClientSequelize.define("vrc_group_member_cache", {
    id: { type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true, allowNull: false },
    groupId: { type: DataTypes.STRING(128), allowNull: false },
    userId: { type: DataTypes.STRING(128), allowNull: false },
    displayName: { type: DataTypes.STRING(500), allowNull: true },
    roleIds: { type: DataTypes.JSON, allowNull: false, defaultValue: [] },
    isRepresenting: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    joinedAt: { type: DataTypes.DATE, allowNull: true },
    membershipStatus: { type: DataTypes.STRING(64), allowNull: true },
    visibility: { type: DataTypes.STRING(64), allowNull: true },
    isSubscribedToAnnouncements: { type: DataTypes.BOOLEAN, allowNull: true },
